...
# these are only shared with horas::specials
our $antecapitulum;
our $antecapitulum1;
our $anterule;
our $hymncontract;
...
sub getrank {
  ...
  if ($hora =~ /(vespera|completorium)/i) {
    ...
    my $vtrank = ($version =~ /(1955|1960|Newcal)/) ? 6 : 7;
    if (
      ($trank[2]//0) >= $vtrank
      && ($crank[2]//0) < 6
    ) {
    ...
    if (@crank && @srank && $crank[2] >= $srank[2]) {
      if ($hora =~ /Vespera/i && $srank[2] == $crank[2] && $crank[2] >= 2) {
        ...
        if ($antecapitulum) {
          my %saint2 = %{setupstring($datafolder, $lang2, $sname)};
          $antecapitulum2 =
...
#  $comrank = 0;
  ...
  # circumstances, even if it doesn't outrank the Sunday numerically.
    ...
    if ($version =~ /1960/) {
      ...
      # With the 1960 rubrics, II. cl. feasts of the
      # Lord and all I.cl. feasts beat II.cl. Sundays.
  ...
  if ($sanctoraloffice) {
    ...
    #if ($version =~ /1960/ && $srank[2] >= 6 && $trank[2] < 6) {$tname = $trank = ''; @trank = ();}
	...
	&& $winner{Rule} !~ /no commemoratio/i
      ...
      $commemoratio1 = ($trank[2]//0) > 1 ? $tname : '';
      $commemoratio = $cname;
      $comrank = $crank[2];
      $cvespera = 4 - $svesp;
      $marian_commem = ($crank[3]//"") =~ /C1[0-9]/;
    ...
    my %w = %{officestring($datafolder, 'Latin', $winner)};
    ...
    if (($hora =~ /matutinum/i || (!$dayname[2] && $hora !~ /Vespera|Completorium/i)) && $rank < 7) {
	  ...
	  || ($version =~ /trident/i
        ...
        $dayname[2] = "Scriptura: $tr0";
      } else {
        $dayname[2] = "Tempora: $tr0";
      }
      $scriptura = $tname;
    }
    ...
    if (transfered($vflag ? $nday : $sday) && $crank !~ /$srank/) {
      ...
      {
        my $laudesonly = ($missa) ? '' : ($climit1960 > 1) ? ' ad Laudes tantum' : '';
...
sub get_sday {
  ...
  my $kalendarkey = sprintf("%02i-%02i", $month, $day);
  return $kalendarkey;
}
...
sub emberday {
  ...
  if (($winner{Rank}//"") =~ /Quatt?uor/i
    || ($commemoratio{Rank}//"") =~ /Quatt?uor/i
    || ($scriptura{Rank}//"") =~ /Quatt?uor/
  ) {
    return 1;
  }
  return 0;
...
sub precedence {
  ...
  if ($winner) {
    ...
    $flag = ($winner =~ /tempora/i && $tvesp == 1) ? 1 : 0;
  ...
  if ($version =~ /1960|Newcal/
    && $winner{Rule} =~ /No Sunday commemoratio/i
    && $dayofweek == 0
  ) {
    $commemoratio = '';
    $commemoratio1 = '';
  ...
  if ($commemoratio) {
    my $flag = ($commemoratio =~ /tempora/i
      && $tvesp == 1);
    %commemoratio = %{officestring($datafolder, $lang1, $commemoratio, $flag)};

    if ($version =~ /1960|Newcal/
      && $winner{Rule} =~ /Festum Domini/
      && $commemoratio{Rule} =~ /Festum Domini/i
    ) {
      $commemoratio = '';
      %commemoratio = ();
      $dayname[2] = '';
    ...
    if ($vespera == $svesp
      && $vespera == 1
      && $cvespera == 3
      && $commemoratio{Rule} =~ /No second Vespera/i
    ) {
      $commemoratio = '';
      %commemoratio = ();
      $dayname[2] = '';
...
sub monthday {
  ...
  my $md = sprintf('%02i%01i-%01i', $m - 1, $weeks + 1, $dow);
  return $md;
}
...
sub transfered {
  ...
  return 0
    if $transfertemp && $str =~ /$transfertemp/i;

  return 0
    if $transfer && $str =~ /$transfer/i;

  foreach my $key (keys %transfer) {
    next
      if $transfer{$key} =~ /Tempora/i
      && $transfer{$key} !~ /Epi1\-0/i;
  ...
  foreach my $key (keys %transfertemp) {
    if ($key !~ /dirge/i
      && $transfertemp{$key} =~ /$str/i
      && ($transfer{$key}//"") !~ /v\s*$/i
    ) {
      return 1;
    }
...
sub expand {
  ...
  # this assigns a temporary global value
  # to our $expand which lasts until return:
